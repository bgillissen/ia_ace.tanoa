...
//-------------------- 7. BRIEFING
	...
	["NewPriorityTarget", "Destroy Anti-Air"] remoteExec ["AW_fnc_globalNotification",0,false];
...
while {_loopVar} do {
	...
	if (!alive priorityObj1) then {
		if (!alive priorityObj2) then {
			...
			showNotification = ["CompletedPriorityTarget", "Anti-Air Battery Neutralised"];
			["CompletedPriorityTarget", "Anti-Air Battery Neutralised"] remoteExec ["AW_fnc_globalNotification",0,false];
